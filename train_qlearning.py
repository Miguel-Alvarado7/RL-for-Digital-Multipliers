"""Entrenamiento de Q-learning off-policy sobre BinaryMathEnv (CPU).

Uso:
    python train_qlearning.py [--bits 2] [--height 2] [--episodes 1000]
                              [--alpha 0.1] [--gamma 0.95] [--seed 42] [--no-plot]

Artifacts en out/qlearning/:
    returns.csv         recompensas por episodio (QL y baseline)
...
from agents.cpu.qlearning import QLearningAgentCPU
...
def main():
    ...
    # Entrenamiento Q-learning. Sembrar np.random para que closed() use la MISMA
    # secuencia de casos de prueba en cada corrida (reproducibilidad).
    np.random.seed(args.seed)
    agent = QLearningAgentCPU(
        n_states=n_states,
        n_actions=n_actions,
        alpha=args.alpha,
        gamma=args.gamma,
    ...
    print(f"Q-learning vs baseline: {eval_returns.mean() - baseline.mean():+.2f}")
    ...
    if not args.no_plot:
        learning_curve(returns, baseline, eval_returns.mean(),
                       ...
                       title="Q-learning off-policy - BinaryMathEnv",
